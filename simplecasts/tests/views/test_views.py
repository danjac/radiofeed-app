...
class TestCoverImage:
    ...
    def test_ok(self, client, mocker):
        mocker.patch(
            "simplecasts.services.covers.fetch_cover_image", return_value=b"ok"
        )
        mocker.patch(
            "simplecasts.services.covers.process_cover_image",
            return_value=mocker.MagicMock(),
        )
        mocker.patch("simplecasts.services.covers.save_cover_image")
    ...
    def test_invalid_fetch(self, client, mocker):
        mocker.patch(
            "simplecasts.services.covers.fetch_cover_image",
            return_value=b"ok",
    ...
    def test_invalid_image(self, client, mocker):
        ...
        mocker.patch(
            "simplecasts.services.covers.save_cover_image",
            side_effect=covers.CoverSaveError(),
        )
        response = client.get(covers.get_cover_url(self.cover_url, 96))
        assert200(response)
