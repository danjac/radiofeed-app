...
class TestPrivateFeeds:
    url = reverse_lazy("private_feeds:private_feeds")
...
class TestRemovePrivateFeed:
    def url(self, podcast):
        return reverse("private_feeds:remove_private_feed", args=[podcast.pk])
    ...
    def test_ok(self, client, auth_user):
        ...
        assert response.url == reverse("private_feeds:private_feeds")
...
class TestAddPrivateFeed:
    url = reverse_lazy("private_feeds:add_private_feed")
