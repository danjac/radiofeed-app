...
class TestSendNewEpisodesEmail:
    def test_no_user(self, db, mailoutbox):

        assert not send_new_episodes_email(1234, timedelta(days=7))
        assert len(mailoutbox) == 0

    def test_no_subscriptions(self, user, mailoutbox):

        assert not send_new_episodes_email(user.id, timedelta(days=7))
        assert len(mailoutbox) == 0
    ...
    def test_sufficient_episodes(self, user, mailoutbox):
        ...
        assert send_new_episodes_email(user.id, timedelta(days=7))
