...
class TestFastCount:
    @pytest.mark.django_db
    def test_count_without_filter_uses_reltuples(self, mocker):
        ...
        assert count == 100
        mock_reltuples.assert_called_once_with("podcasts_podcast")

    @pytest.mark.django_db
    def test_count_with_filter_uses_standard_count(self):
        ...
        qs = Podcast.objects.filter(active=True)
        count = qs.count()
        assert count == 3

    @pytest.mark.django_db
    def test_count_reltuples_returns_estimate(self):
        """count_reltuples should return estimated row count."""
        from django.db import connection

        from radiofeed.db.fast_count import count_reltuples

        PodcastFactory.create_batch(3)
        with connection.cursor() as cursor:
            cursor.execute(f"ANALYZE {Podcast._meta.db_table}")
        count = count_reltuples("podcasts_podcast")
        assert count >= 3
