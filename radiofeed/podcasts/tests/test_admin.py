...
class TestPodcastAdmin:
    ...
    def test_parse_podcast_feed_inactive(
        self, mock_parse_feed, podcast, podcast_admin, req
    ):
...
class TestPubDateFilter:
    ...
    def test_recent(self, db, podcast_admin, req):
        now = timezone.now()
        PodcastFactory(pub_date=now - timedelta(days=30))
        recent = PodcastFactory(pub_date=now - timedelta(days=3))
        f = PubDateFilter(req, {"pub_date": "recent"}, Podcast, podcast_admin)
        qs = f.queryset(req, Podcast.objects.all())
        assert qs.count() == 1
        assert recent in qs
...
class TestSubscribedFilter:
    ...
    def test_promoted_filter_true(self, podcasts, podcast_admin, req):
