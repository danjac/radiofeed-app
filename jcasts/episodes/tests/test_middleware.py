...
from jcasts.episodes.middleware import Player, PlayerMiddleware
...
class TestPlayer:
    ...
    def test_remove_if_none(self, player):
        assert player.remove() is None

    def test_remove_if_not_none(self, player):
        player.set(self.episode_id)

        assert player.remove() == self.episode_id
        assert player.get() is None
